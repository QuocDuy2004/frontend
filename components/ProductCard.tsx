    ...
    <Pressable onPress={onPress} className={`rounded-2xl border border-gray-100 bg-white p-3 ${horizontal ? 'flex-row gap-4' : ''}`}>
      ...
      <View className={`flex-1 gap-1 ${horizontal ? 'justify-center' : 'mt-3'}`}>
        ...
        <View className="flex-row items-center gap-1">
          ...
          <Text className="text-[10px] text-gray-500">{product.rating || 5} • {product.reviewCount || 0} danh gia</Text>
        </View>
        <View className="mt-1 flex-row flex-wrap items-baseline gap-1">
          <Text className="text-sm font-black text-red-500">{product.discountPrice.toLocaleString('vi-VN')}d</Text>
          <Text className="text-[10px] text-gray-400 line-through">{product.originalPrice.toLocaleString('vi-VN')}d</Text>
        </View>
      </View>
    </Pressable>
  );
